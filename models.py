...
class OrderLine(db.Model):
    ...
    # สถานะการพิมพ์รายงานสินค้าน้อย (Low Stock Report)
    printed_lowstock = db.Column(db.Integer, default=0)
    printed_lowstock_at = db.Column(db.DateTime)
    printed_lowstock_by = db.Column(db.String(64))
    lowstock_round = db.Column(db.Integer)

    # สถานะการพิมพ์รายงานไม่มีสินค้า (No Stock Report)
    printed_nostock = db.Column(db.Integer, default=0)
    printed_nostock_at = db.Column(db.DateTime)
    printed_nostock_by = db.Column(db.String(64))
    nostock_round = db.Column(db.Integer)

    # สถานะการพิมพ์รายงานสินค้าไม่พอส่ง (Not Enough Report)
    printed_notenough = db.Column(db.Integer, default=0)
    printed_notenough_at = db.Column(db.DateTime)
    printed_notenough_by = db.Column(db.String(64))
    notenough_round = db.Column(db.Integer)

    # Scan Barcode
    scanned_at = db.Column(db.DateTime)  # เวลาที่ Scan Barcode
    scanned_by = db.Column(db.String(64))  # ผู้ Scan

    # Allocation Status (สถานะการจัดสรรสต็อก / บิลเปล่า)
    allocation_status = db.Column(db.String(50))  # BILL_EMPTY, READY_ACCEPT, SHORTAGE, etc.
