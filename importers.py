...
from models import db, Shop, Product, Stock, Sales, OrderLine, SkuPricing, BrandControl, PlatformFeeSetting, MarketItem, MarketPriceLog, PriceImportOp, SupplierSkuMaster
...
def get_or_create_shop(platform, shop_name, is_system_config=False):
    platform = normalize_platform(platform)
    name = clean_shop_name(shop_name)
    shop = Shop.query.filter_by(name=name).first()
    if not shop:
        shop = Shop(platform=platform, name=name, is_system_config=is_system_config)
        db.session.add(shop)
        db.session.commit()
    return shop
