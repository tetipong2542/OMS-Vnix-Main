...
def compute_allocation(session, filters:dict):
    ...
    for ol, shop, prod, stock, sales in all_data:
        ...
        # [แก้ไข] อ่านค่า allocation_status จาก DB (ถ้ามี) เพื่อรักษาสถานะเช่น BILL_EMPTY
        ...
        # [สำคัญ] ยกเว้น BILL_EMPTY ต้องแสดงเสมอ แม้จะ is_packed=True (เพราะ Scan แล้ว)
        if filters.get("active_only"):
            if db_allocation_status != "BILL_EMPTY":
        ...
        row_data = {
            ...
            "is_issued": is_issued,
            "allocation_status": db_allocation_status,  # [แก้ไข] อ่านจาก DB แทนที่จะเป็นค่าว่าง
    ...
    for sku, arr in by_sku.items():
        ...
        for r in arr:
            # [เพิ่ม] ถ้า allocation_status เป็น BILL_EMPTY อยู่แล้ว (จาก DB) ให้คงค่าไว้ ไม่ต้องคำนวณใหม่
            if r["allocation_status"] == "BILL_EMPTY":
                # บิลเปล่าไม่ต้องตัดสต็อก เพราะไม่มีสินค้าจริง
                continue
